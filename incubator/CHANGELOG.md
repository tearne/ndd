...
- The installer ships the whole checkout except development residue (`changes/`, git's files, `.claude/`, the repository's own entry files, `README.md` and the installer itself), keeping scripts executable, so new files ship without a list to maintain.
