...
- **`Tooling` authored (2026-08-17); root reframed to reality; tooling ambition parked.** Sync-rule catch: the root promised "the viewer and index that serve both", but neither exists. `Tooling` (`g8l`) authored to describe present reality — the method rides generic markdown tooling (marksman in Helix; `gd` walks parent↔child via name-anchored links, `Space+s` symbol picker jumps to any node by name; scaffolding folds away), nothing bespoke required. Root bullet retuned "viewer and index" → "the generic markdown tooling the format is designed to exploit"; tree overview `Tooling (TODO)` → `Tooling`. The unbuilt browser-viewer/index ambition captured as a new parked change `tooling-development.md` (references *Tooling (g8l)*, points at `enhancement-discussion.md` P3) so it isn't lost. **Every node in the map is now authored — no stubs remain.** Remaining change work: go-live only (delete the four migrated docs, repoint `CLAUDE.md` from `AGENT.md` to `BOOTSTRAP.md`).

- **Dropped inaccurate folding claim from `Tooling` (2026-08-17).** Helix has no code folding, so the scaffolding block doesn't "fold away" there — that affordance belongs to a future bespoke viewer (parked in `tooling-development.md`), not present reality. Sentence removed.

- **GAP found: Sync rule and Engagement rule never transcribed (2026-08-17).** Both governing map-edit rules live only in `MAP-GUIDANCE.md` (Sync: "map describes what exists, not what is proposed"; Engagement: "every map edit is negotiated, one node at a time"). `Map Maintenance` (`t7v`) carried the restructuring signals and the ambiguity test but not these two. **Blocks go-live** — deleting `MAP-GUIDANCE.md` would lose them. Must add a home in the Specification subtree (candidate: a new "Editing the Map" node holding both rules) before the doc deletions. Surfaced to user; placement under discussion.

- **`Edit Governance` subtree authored, closing the Sync/Engagement gap (2026-08-17).** New Specification child `Edit Governance` (`e7m`, renamed from working title "Editing the Map") frames the three questions every map edit answers — *what* (`Sync Rule` `s5y`), *how* (`Engagement Rule` `n3g`), *when* (`Map Maintenance` `t7v`, re-parented from `Specification`, id kept — a live ID-durability demo). `Sync Rule` reworked to a two-faced framing (edit to match reality any time, never ahead). Active-change exemption consolidated once into the parent (was repeated across both rules), cross-linked to `Gates and Permissions`. `Map Maintenance` tightened by the user in its new home; the **only-child preference bullet was deliberately dropped** (judged low-value/near-obvious, not relocated) — a conscious omission from the migrated `MAP-GUIDANCE.md` content. `Specification` child links + tree overview updated. Go-live is now unblocked.
