...
Costs to weigh: two roots and two Contents; cross-file links (`meta.md#node`); every tool that walks the map must know both files; Map Structure, META, Bootstrapping and the installer all assume one file. The existing META subtree would lift mechanically into the new root. Plan alongside 230 and 240.
